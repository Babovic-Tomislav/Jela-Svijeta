...
class MealsSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $ingreadients = Ingredients::factory()->times($ingredientNumber)
            ->create();
        $category = Category::factory()->times($categoryNumber)->create();
        $tags = Tags::factory()->times($tagNumber)->create();
        $meals = Meals::factory()->times($mealNumber)->create()->each(
            function ($meal) use ($ingreadients, $tags, $category) {
                $meal->tags()->sync(
                    $tags->random(rand(2, 5))->pluck('id')->all()
                );
                $meal->Ingredients()->sync(
                    $ingreadients->random(rand(3, 5))->pluck('id')->all()
                );
                if (rand(0, 1)) {
        ...
        foreach ($meals->random(10) as $meal) {
            $randomDate = Faker::create()->dateTimeBetween(
                $meal->created_at,
                'now'
            );
